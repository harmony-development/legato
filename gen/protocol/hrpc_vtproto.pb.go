...
func (m *Error) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.HumanMessage) > 0 {
		i -= len(m.HumanMessage)
		copy(dAtA[i:], m.HumanMessage)
		i = encodeVarint(dAtA, i, uint64(len(m.HumanMessage)))
		i--
		dAtA[i] = 0x12
	}
...
func (m *RetryInfo) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.RetryAfter != 0 {
		i = encodeVarint(dAtA, i, uint64(m.RetryAfter))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
...
func (m *Error) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field HumanMessage", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.HumanMessage = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
	...
	for iNdEx < l {
		...
		case 1:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.RetryAfter |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
